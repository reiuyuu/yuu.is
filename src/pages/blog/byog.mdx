...
import { IMG, Ref, Note, FootNotes } from '@/components/mdx-blocks'
import randomMapExample from '../../../public/images/byog/random_map_example.png'
import menu from '../../../public/images/byog/menu.png'
import gameUI from '../../../public/images/byog/game_ui.png'
...
export const meta = {
  ...
  description:
    "CS 61B Project 2 involves creating a 2D tile-based game engine for generating explorable worlds. Implements a world generation algorithm based on Bob Nystrom's and covers pseudorandom and saving / loading using Java's Serializable interface.",
}
...
[Project 2: CS61BYoG](https://sp18.datastructur.es/materials/proj/proj2/proj2) is the third project (there is Project 0) in [CS 61B Data Structures, Spring 2018](https://sp18.datastructur.es/index.html). Its specific theme is **Large Scale Design and Testing**.<Ref id={1} /> As described in the material, the task is to create an engine for generating explorable worlds — that is, to design and implement a 2D tile-based game.

> By "tile-based", we mean the world for your game will consist of a 2D grid of tiles. By "game" we mean that the player will be able to walk around and interact with the world.<Ref id={2} />
...
<IMG
  src={randomMapExample}
  alt='random map example'
  desc='random map example'
/>
...
> 4. Remove all of the dead ends.<Ref id={3} />
...
After filling in the rooms and mazes, find connections between them. We treat each region of tiles as a single vertex and each connector as an edge between them,<Ref id={3} /> and then it is all about finding a spanning tree:
...
> 5. If there are still connectors left, go to #2.<Ref id={3} />
...
> In the second phase of the game, you'll add the ability for the user to actually play the game, and will also add user interface (UI) elements to your game to make it feel more immersive and informative.<Ref id={2} />
...
<IMG src={menu} alt='game menu' desc='game menu' />
...
<IMG src={gameUI} alt='game UI' desc='game UI' />
...
> Your game must have the ability to save the game while playing and load the game in the exact state of the most recent save after quitting and opening the game back up.<Ref id={2} />

The technique to accomplish this is the Java interface `Serializable`. What it does is basically translate a data structure or object state into a format that can be stored or transmitted. Implementing the interface marks the class as "okay to serialize", and Java then handles serialization internally.<Ref id={4} />
...
> The goal of this project is to teach you how to handle a larger piece of code with little starter code in the hopes of emulating something like a product development cycle.<Ref id={2} />

<FootNotes>
  <Note
    id={1}
    src='https://sp18.datastructur.es/about.html'
    text='About | CS 61B Spring 2018'
  />
  <Note
    id={2}
    src='https://sp18.datastructur.es/materials/proj/proj2/proj2'
    text='Project 2: CS61BYoG | CS 61B Spring 2018'
  />
  <Note
    id={3}
    src='https://journal.stuffwithstuff.com/2014/12/21/rooms-and-mazes/'
    text='Rooms and Mazes: A Procedural Dungeon Generator'
  />
  <Note
    id={4}
    src='https://en.wikipedia.org/wiki/Serialization'
    text='Serialization - Wikipedia'
  />
</FootNotes>
